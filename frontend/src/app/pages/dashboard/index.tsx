...
interface QaLiveFeedCallWidget {
  data: {
    ...
    earlyDecline: number;
    totalClosersCall: number;
    totalSDRsCall: number;
    Verification: number;
    caseManagers: number;
...
const PAGE_TITLES: Record<string, [string, string]> = {
  'departments-team-members': ['Sales', 'City Financial'],
  'customer': ['Customer Service', 'City Financial'],
  'verification': ['Verification', 'City Financial'],
  'caseManager': ['Case Manager', 'City Financial'],
  'rtc': ['RTC', 'City Financial'],
  'nsf': ['NSF', 'City Financial'],
  'finance': ['Finance', 'City Financial'],
...
const Dashboard: React.FC = () => {
  ...
  const [deptAndTeams, setdeptAndTeams] = useState<string | null>(null);
  ...
  }, [callOutcome, callFlag, callScore, callDepartment, deptAndTeams]);
  useEffect(() => {
  ...
  const fetchLiveFeedData = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/dashboard/calls?dateFrom=${dateFrom}&dateTo=${dateTo}&flags=${callFlag}&agentDept=${callDepartment}&score=${callScore}&outcome=${callOutcome}&page=${page}&pageSize=${ROWS_PER_PAGE}${['departments-team-members','customer','verification','finance','rtc','caseManager','nsf'].includes(activePage) ? `&deptAndTeams=${deptAndTeams}` : ''}`
      );
  ...
  const fetchLiveFeedwidgetData = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/dashboard/live-feed-widget-data?dateFrom=${dateFrom}&dateTo=${dateTo}${['departments-team-members','customer','verification','finance','rtc','caseManager','nsf'].includes(activePage) ? `&deptAndTeams=${deptAndTeams}` : ''}`
      );
      if (!response.ok) throw new Error('Network error syncing calls database.');
  ...
  const handleDateRangeApply = () => {
    ...
    if (activePage === 'departments-team-members') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'customer') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'verification') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'caseManager') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'finance') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'rtc') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
    if (activePage === 'nsf') { fetchLiveFeedData(1); fetchLiveFeedwidgetData(); }
  ...
  const renderCallRows = () => {
    ...
    const rows = allCalls.filter(c => c.agentName != null).map(c => {
      ...
      return (
        <div
          key={c.id}
          style={{ display: 'grid', gridTemplateColumns: `260px 70px 90px 200px ${['departments-team-members'].includes(activePage) ? `575px` : '130px'} 1fr`, alignItems: 'center', padding: '6px 12px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.1s' }}
  ...
  const renderSidebarLB = () => {
    ...
    if (!qaLiveFeedCallWidget?.agentData || qaLiveFeedCallWidget.agentData.length === 0) {
      ...
      return (
        <div style={{ padding: '10px 0', fontSize: '11px', color: 'var(--text3)' }}>
          No agent data available for this date range
        </div>
      );
    }
    const sortedAgents = [...qaLiveFeedCallWidget.agentData].sort((a, b) => b.avgScore - a.avgScore);

    return sortedAgents.map((a, i) => {
      ...
      return (
        <div key={a.id || a.agentName} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 0', borderBottom: '1px solid var(--border)' }}>
    ...
    <div className="app">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <div className="nav-section">
            ...
            <div className={`nav-item ${activePage === 'qa-live' ? 'active' : ''}`} onClick={() => { setdeptAndTeams(null); setActivePage('qa-live'); }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
              Live Feed
              <span className="nav-badge">{qaLiveFeedCallWidget?.data.sidebarTotalCalls}</span>
            </div>
            <div className='nav-item' onClick={() => setIsOpen(!isOpen)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="7" y="3" width="10" height="18" rx="2" /><path d="M10 7h.01M14 7h.01M10 11h.01M14 11h.01M10 15h.01M14 15h.01" /><path d="M12 21v-2" /><path d="M8 19h8" /><circle cx="8" cy="21" r="1" /><circle cx="12" cy="21" r="1" /><circle cx="16" cy="21" r="1" /></svg>
              Departments & Team Members
              <span style={{ marginLeft: "10px" }}>{isOpen ? "▲" : "▼"}</span>
            </div>
            {isOpen && (
              <div className="sub-menu">
                <div className={`sub-menu-item ${activePage === 'departments-team-members' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('All'); setActivePage('departments-team-members'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"><path d="M3 17l6-6 4 4 8-8" /><path d="M15 7h6v6" />
                  </svg>
                  Sales
                  <span className="nav-badge">{((Number(qaLiveFeedCallWidget?.data?.totalClosersCall ?? 0)) + (Number(qaLiveFeedCallWidget?.data?.totalSDRsCall ?? 0))).toLocaleString()}</span></div>
                <div className={`sub-menu-item ${activePage === 'customer' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('customer'); setActivePage('customer'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"><path d="M4 12a8 8 0 0 1 16 0" /> <rect x="2" y="12" width="4" height="7" rx="2" /> <rect x="18" y="12" width="4" height="7" rx="2" /><path d="M6 19a2 2 0 0 0 2 2h4" />
                  </svg>
                  Customer Service
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.customerService}</span>
                </div>
                <div className={`sub-menu-item ${activePage === 'verification' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('verification'); setActivePage('verification'); }}>
                  ...
                  Verification
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.Verification}</span>
                </div>
                <div className={`sub-menu-item ${activePage === 'caseManager' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('caseManager'); setActivePage('caseManager'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round"><rect x="6" y="4" width="12" height="16" rx="2" /><path d="M9 4.5h6" /><path d="M9 9h6" /><path d="M9 13h6" /><path d="M9 17h4" />
                  </svg>
                  Case Management
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.caseManagers}</span>
                </div>
                <div className={`sub-menu-item ${activePage === 'rtc' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('rtc'); setActivePage('rtc'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20" /><path d="M16 6c0-1.7-1.8-3-4-3s-4 1.3-4 3 1.8 3 4 3 4 1.3 4 3-1.8 3-4 3-4-1.3-4-3" /><path d="M18 15l4 4" />
                    <path d="M22 15l-4 4" />
                  </svg>
                  RTC
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.rtc}</span>
                </div>
                <div className={`sub-menu-item ${activePage === 'nsf' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('nsf'); setActivePage('nsf'); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20" /><path d="M16 6c0-1.7-1.8-3-4-3s-4 1.3-4 3 1.8 3 4 3 4 1.3 4 3-1.8 3-4 3-4-1.3-4-3" /><path d="M18 15l4 4" />
                    <path d="M22 15l-4 4" />
                  </svg>
                  NSF
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.nsf}</span>
                </div>
                <div className={`sub-menu-item ${activePage === 'finance' ? 'active' : ''}`} onClick={() => { setdeptAndTeams('finance'); setActivePage('finance'); }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  Finance
                  <span className="nav-badge">{qaLiveFeedCallWidget?.data.finance}</span>
                </div>
              </div>
            )}
      ...
      <div className="main">
        ...
        <div className="content">
          {/* QA Live Page */}
          <div className={`page ${['qa-live', 'departments-team-members','customer','verification','finance','rtc','caseManager','nsf'].includes(activePage) ? 'active' : ''}`}>
